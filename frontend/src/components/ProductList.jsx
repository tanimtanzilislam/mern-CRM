export default function ProductList({ products, onEdit, onDelete, hasSearch, canManage = false }) {
    ...
    <table className="customer-table">
      ...
      <tbody>{products.map((product) => (
        <tr key={product._id}>
          ...
          <td className="actions">{canManage && <><button className="table-action" onClick={() => onEdit(product)}>Edit</button><button className="table-action danger" onClick={() => onDelete(product._id)}>Delete</button></>}</td>
        </tr>
      ))}</tbody>
    </table>
  );
